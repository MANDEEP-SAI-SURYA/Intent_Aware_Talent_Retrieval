...
def main():
    ...
    qdrant_dir = project_dir / "RAG" / "qdrant_d_b"
    qdrant_zip = project_dir / "RAG" / "qdrant_d_b.zip"
    if not qdrant_dir.exists() and qdrant_zip.exists():
        print(f"Extracting local Qdrant database from: {qdrant_zip}")
        with zipfile.ZipFile(qdrant_zip, "r") as zip_ref:
            zip_ref.extractall(qdrant_zip.parent)

    # Defaults are repo-relative so `python main.py` works on Windows and Colab.
    JD_FILE = os.getenv(
        "JD_FILE",
        str(project_dir / "data" / "data2" / "job_description.docx")
    )
    CANDIDATES_FILE = os.getenv(
        "CANDIDATES_FILE",
        str(project_dir / "data" / "data2" / "candidates.jsonl")
    )
    COLLECTION_NAME = os.getenv("COLLECTION_NAME", "Candidates")
    OUTPUT_FILE = os.getenv(
        "OUTPUT_FILE",
        str(project_dir / "submission.csv")
    )
